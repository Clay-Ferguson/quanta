...
# Grafana Alloy Stack Environment Variables - PRODUCTION Environment
...
export GRAFANA_ENV_SUFFIX="prod"    # Suffix for container names to identify environment

# Port configurations for Grafana services - PRODUCTION ports (different from dev)
export GRAFANA_PORT="3002"           # Grafana UI port (dev uses 3000, local uses 3001)
export LOKI_PORT="3102"             # Loki API port (dev uses 3100, local uses 3101)
export ALLOY_HTTP_PORT="12347"      # Alloy HTTP server port (dev uses 12345, local uses 12346)
export ALLOY_OTLP_PORT="4320"       # Alloy OTLP receiver port (dev uses 4318, local uses 4319)
