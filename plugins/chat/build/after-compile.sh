...
# Next copy plugins/docs/chat to dist/plugins/docs/chat
mkdir -p dist/docs/extensions/chat
if ! cp -r plugins/chat/docs/* dist/docs/extensions/chat/ 2>/dev/null; then
    error_and_pause "Failed to copy plugins/chat/docs/* to dist/docs/extensions/chat/"
fi
