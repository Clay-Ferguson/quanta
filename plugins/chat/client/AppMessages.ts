...
import { ChatMessage, ChatMessageIntf, DeleteMessage_ReqInfo, GetMessageIdsForRoom_ResInfo, GetMessagesByIds_Response, MessageStates, SendMessages_ReqInfo } from "../common/CommonTypes.ts";
...
export class AppMessages {
    ...
    deleteMessage = async (messageId: string) => {
        ...
        if (messageIndex !== undefined && messageIndex >= 0) {
            ...
            await httpClientUtil.secureHttpPost<DeleteMessage_ReqInfo, any>('/api/delete-message', {
                messageId,
    ...
    resendFailedMessages = async (roomName: string, messages: ChatMessage[]): Promise<ChatMessage[]> => {
        ...
        try {
            ...
            const response = await httpClientUtil.secureHttpPost<SendMessages_ReqInfo, any>(
    ...
    loadRoomMessages = async (roomId: string): Promise<ChatMessage[]> => {
        ...
        if (gs().chatSaveToServer) {
            ...
            try {
                ...
                const respIds: GetMessageIdsForRoom_ResInfo = await httpClientUtil.httpGet(`/api/rooms/${encodeURIComponent(roomId)}/message-ids?daysOfHistory=${chatDaysOfHistory}`);
